...
def run(label: str, command: list[str], timeout: int = 7200) -> dict:
    print(f"\n[{label}]", flush=True)
...
def final_summary(target: str, history: list[dict]) -> None:
    ...
    lines = ["# VulnScope Summary", "", f"Target: `{target}`", f"Version: `{VERSION}`", "", "## Steps"]
    for item in history:
...
def run_autonomous_engine(target: str, args: argparse.Namespace) -> dict:
    cmd = [
        sys.executable,
        "-m",
...
def run_modern_react_engine(target: str, args: argparse.Namespace) -> dict:
    ...
    return run("Modern Browser Surface Engine", cmd, timeout=7200)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--render-js", action="store_true", help="Alias for --browser in the autonomous engine and Playwright rendering in the modern surface engine.")
    parser.add_argument("--modern-react", action="store_true", help="Run the standalone browser surface engine from core.react_loop instead of the autonomous CAI runtime.")
