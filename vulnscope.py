...
╔════════════════════════════════════════════════════════════════════╗
║                         VulnScope Ultimate                       ║
║  Autonomous Crawl → AI Planner → Safe Tests → Evidence → Report   ║
╚════════════════════════════════════════════════════════════════════╝
"""
...
def confirm(target: str, yes: bool) -> None:
    ...
    if not yes and os.getenv("VULNSCOPE_AUTHORIZED", "0") != "1":
        ...
        print("- Hidden routing and detection-bypass behavior are not used.")
...
def final_summary(target: str, history: list[dict]) -> None:
    ...
    payload = {
        ...
        "interface": "kali_cli",
        "autonomous_scan_engine": True,
        "ollama_planner": True,
        "resume_supported": True,
        "evidence_store": True,
        "request_budget": True,
        "browser_crawler_optional": True,
    ...
    lines = [
        ...
        f"Target: `{target}`",
        "",
        "Autonomous scan engine: `true`",
        "Ollama planner: `true`",
        "Resume supported: `true`",
        "Evidence store: `true`",
        "Request budget: `true`",
        "Optional browser crawler: `true`",
...
def run_agentic(target: str, args: argparse.Namespace) -> dict:
    ...
    history.append(run("Autonomous Scan Engine", engine_cmd, timeout=3600))
    ...
    return {"label": "VulnScope 1.8 Autonomous Pipeline", "ok": ok, "exit_code": 0 if ok else 1, "steps": history}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="VulnScope autonomous safe scanner")
