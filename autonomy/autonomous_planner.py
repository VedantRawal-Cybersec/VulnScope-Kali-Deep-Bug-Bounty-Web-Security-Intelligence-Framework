...
def build_plan(target: str, policy: AutonomyPolicy, has_har: bool = False) -> list[AutonomousStep]:
    ...
    if policy.allows_stage("daily_update"):
        steps.append(AutonomousStep("daily_update", "daily_update", "Refresh curated tools, local templates, healthcheck state, and latest defensive intelligence once per day", "daily_update_cli.py --yes", required=False))
    steps += [
        AutonomousStep("safe_discovery", "safe_discovery", "Find real misconfigurations and review candidates using low-impact evidence collection", "safe_discovery.SafeDiscoveryRunner"),
