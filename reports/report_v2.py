...
def build_report_v2(target: str | None = None) -> dict[str, Path]:
    ...
    title_target = target or agent.get("target") or safe_discovery.get("target") or "authorized-target"
    md = _markdown(title_target, agent, council, workflow, quality, safe_discovery)
    json_out = OUT / "executive-report-v2.json"
    md_out = OUT / "executive-report-v2.md"
    json_out.write_text(json.dumps({"target": title_target, "agent_summary": agent, "safe_discovery": safe_discovery, "quality": quality, "council": council}, indent=2, ensure_ascii=False), encoding="utf-8")
    md_out.write_text(md, encoding="utf-8")
    return {"markdown": md_out, "json": json_out}


def _markdown(target: str, agent: dict[str, Any], council: str, workflow: str, quality: dict[str, Any], safe_discovery: dict[str, Any]) -> str:
