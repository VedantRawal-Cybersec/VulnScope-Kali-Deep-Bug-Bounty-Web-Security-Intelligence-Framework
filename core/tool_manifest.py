...
class ToolManifest:
    ...
    Supports explicit `run` templates generated by the AI Tool Auto-Configurator.
    If `run` is present, it is used exactly. Otherwise the older entry_point +
    safe_flags compatibility path is used.
    """
    ...
    def _generate_heuristic(self) -> dict[str, Any]:
        ...
        for candidate in ["main.py", "cli.py", "run.py", "scan.py", "scanner.py", "app.py", "run.sh", "setup.sh"]:
            if (self.tool_path / candidate).exists():
                entry = candidate
                break
        return self._normalize({"name": name, "entry_point": entry, "phase": phase, "output_format": "jsonl" if lower in {"nuclei", "katana", "httpx", "subfinder", "naabu"} else "plain", "safe_flags": [], "timeout": 240, "dependencies": []})
    ...
    def _normalize(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(run, list):
            normalized["run"] = [str(item) for item in run]
        else:
            normalized["run"] = str(run)
    ...
    def get_command(self, target: str, additional_flags: list[str] | None = None) -> list[str]:
        explicit_run = self.data.get("run")
        if explicit_run:
            if isinstance(explicit_run, list):
                cmd = [str(item).replace("{target}", target) for item in explicit_run]
            else:
                cmd = [item.replace("{target}", target) for item in shlex.split(str(explicit_run))]
            if additional_flags:
    ...
    def to_registry_manifest(self) -> dict[str, Any]:
        ...
        return {"name": self.data["name"], "version": str(self.data.get("version") or "unknown"), "phase": self.data["phase"], "install": self.data.get("dependencies", []), "run": command, "arguments": [{"name": "target", "description": "Target URL", "required": True}], "output_parser": self.data["output_format"], "metadata": {"manifest_json": True, "manifest_path": str(self.manifest_path)}}
