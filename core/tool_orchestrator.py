...
SAFE_ACTIVE_CATEGORIES = {
    "Safe Canary Reflection Testing",
...
class UltimateToolOrchestrator:
    ...
    def _update_dashboard(self, spec: ToolSpec, index: int, total: int, status: str, evidence: str = "") -> None:
        ...
        self.dashboard.update(
            ...
            safety_status="100-tool registry • allowlisted actuators • no exploit payloads • no target data modification",
        )
    ...
    def _run_one(self, spec: ToolSpec, index: int, total: int) -> ToolResult:
        ...
        if not _mode_allows(self.scan_mode, spec.required_scan_mode):
            return ToolResult(
                ...
                "skipped",
                0,
                skipped_reason=f"requires {spec.required_scan_mode}; current mode is {self.scan_mode}",
                actuator=spec.run_function,
            )
        ...
        try:
            ...
            if _is_error_result(raw):
                result = ToolResult(
                    ...
                    "failed",
                    runtime_ms,
                    output_count=_output_count(raw),
                    actuator=spec.run_function,
                    error=_summary(raw),
            ...
            result = ToolResult(
                ...
                output_count=_output_count(raw),
                actuator=spec.run_function,
                message="completed using safe integrated actuator output",
                evidence_summary=_summary(raw),
                cache_reused=cache_reused,
            )
    ...
    def write_reports(self, out: Path) -> dict[str, str]:
        ...
        lines = [
            "# VulnScope 100-Tool Orchestration Matrix",
            "",
        ...
        for result in payload["results"]:
            lines.append(
                f"- `{result['tool_id']}` **{result['tool_name']}** — category=`{result['category']}` "
                f"status=`{result['status']}` actuator=`{result['actuator']}` runtime_ms=`{result['runtime_ms']}` "
                f"output_count=`{result['output_count']}` cache_reused=`{result['cache_reused']}`"
            )
        ...
        return {
            "tool_matrix_json": str(matrix_json),
            "tool_matrix_md": str(matrix_md),
            "tool_registry_100_json": str(registry_json),
        }
