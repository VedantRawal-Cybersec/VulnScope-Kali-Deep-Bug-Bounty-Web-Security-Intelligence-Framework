...
class PhaseScheduler:
    ...
    def _dash(self, *, phase: str, tool_id: str, status: str, target: str, message: str) -> None:
        ...
        if self.dashboard is not None and hasattr(self.dashboard, "event"):
            self.dashboard.event("INFO" if status not in {"failed", "timed_out"} else "ERROR", message)
    ...
    def _finding_from_observation(self, tool: Any, payload: dict[str, Any], target: str, output_path: str) -> dict[str, Any]:
        ...
        return {
            "id": "external_" + str(abs(hash(json.dumps(payload, sort_keys=True, default=str))))[:12],
            "title": f"{tool.name}: {title}",
            "status": "Manual Review Lead",
            "severity": severity,
            "confidence": 70,
            "category": "External Tool Observation",
            "affected_url": affected,
            "parameter": None,
            "evidence": json.dumps(payload, ensure_ascii=False, default=str)[:2000],
            "impact": "A registered external tool produced output that should be manually reviewed before reporting.",
            "recommendation": "Review the saved stdout/stderr and validate the observation in the authorized scope.",
            "safe_probe": tool.tool_id,
            "reproduction_steps": [
                "Run the same VulnScope scan with dynamic tools enabled.",
                f"Review dynamic tool output: {output_path}",
                "Manually validate impact before reporting.",
            ],
        }
    ...
    def run_phase(self, phase: str, *, target: str, confirm: bool = False, timeout: int = 300) -> list[ScheduledToolRun]:
        ...
        for tool in tools:
            ...
            if MODE_RANK.get(self.scan_mode, 0) < MODE_RANK.get(required_mode, 0):
                run = ScheduledToolRun(tool_id=tool.tool_id, phase=phase, status="skipped", started_at=started, finished_at=time.time(), elapsed_ms=0, error=f"requires {required_mode} mode")
                self.runs.append(run)
                results.append(run)
                self._dash(phase=phase, tool_id=tool.tool_id, status="skipped", target=target, message=f"Skipping {tool.name}: requires {required_mode}")
                continue
            if not tool.approved_for_run or not tool.run:
                ...
                run = ScheduledToolRun(tool_id=tool.tool_id, phase=phase, status="skipped", started_at=started, finished_at=time.time(), elapsed_ms=0, error=reason)
                self.runs.append(run)
                results.append(run)
                self._dash(phase=phase, tool_id=tool.tool_id, status="skipped", target=target, message=f"Skipping {tool.name}: {reason}")
                self._log("Skipping dynamic tool", data={"tool_id": tool.tool_id, "reason": reason})
                continue
    ...
    def run_all(self, *, target: str, confirm: bool = False, timeout: int = 300) -> dict[str, Any]:
        ...
        return self.write_summary(extra={"target": target, "grouped": grouped, "scan_mode": self.scan_mode})
